...
| 7 | Interop stress test | **Done 2026-09-22 - no fallback needed.** `TripDetailScreen` is now the Phase 1 shape: GMSMapView as the first item of a `LazyColumn` of stops + a `ModalBottomSheet` with a notes `OutlinedTextField`. Verified on the iPhone 17 Pro simulator with an 8-stop trip: drag on the map pans the map (page does not scroll); drag on the list scrolls the page and the native map scrolls/clips with it; row tap and marker tap both open the sheet; the Compose scrim dims the UIKit map beneath; the software keyboard raises the sheet (`imePadding`) and typing lands; drag-to-dismiss clears the selection. **Frame drops: not measurable on the simulator** (Instruments: "Hitches is not supported on this platform") - measure on a device before Phase 1 sign-off. Same screen verified on the Android emulator (sheet + keyboard). |
