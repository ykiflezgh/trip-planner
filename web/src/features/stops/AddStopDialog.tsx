...
function PlaceForm({ onAdd }: { onAdd: (p: PlacePick, durationMin: number, notes: string) => void }) {
  ...
  // A search for the current query has completed: cleared when the query changes, the list is dismissed or a
  // place is picked, so an empty result set can be told apart from "not searched yet" and announced.
  ...
  useEffect(() => {
    ...
    // A response that lands after the query moved on (or the list was dismissed) must not overwrite the newer state.
    let stale = false
    const handle = setTimeout(async () => {
      try {
        ...
        setSearched(true)
        setError(null)
      } catch (e) { if (!stale) setError(`Search failed: ${String((e as Error).message ?? e)}`) }
    }, 200)
  ...
  const choose = async (s: Suggestion) => {
    ...
    try {
      ...
      setQuery(place.displayName ?? s.primary); setItems([]); setActive(-1); setSearched(false)
  ...
  const onKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    else if (e.key === 'Escape') { e.preventDefault(); e.stopPropagation(); setItems([]); setActive(-1); setSearched(false) } // dismissed, not "no suggestions"
  }
  const status = busy ? 'Loading the place…' : pick ? `${pick.name} selected` : visible.length > 0 ? `${visible.length} suggestion${visible.length === 1 ? '' : 's'}` : searched ? `No suggestions for "${query.trim()}"` : ''
  ...
  return (
    <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); if (pick) onAdd(pick, duration, notes) }}>
      <label className="block text-sm">Search a place
        <input className={field} value={query} onChange={(e) => { setQuery(e.target.value); setPick(null); setActive(-1); setSearched(false) }} onKeyDown={onKey} placeholder="Louvre" autoFocus autoComplete="off"
      ...
      {/* Persistent live regions (WCAG 4.1.3): the suggestion count, empty result and pick for screen readers, the error for everyone. */}
