...
export interface Stop { id: string; name: string; kind: string; day: number; order: string; durationMin: number; fixedStart: string | null; address: string; lat: number | null; lng: number | null }
export interface TripDetailState {
  ...
  canEdit: boolean; isOwner: boolean; pendingSync: boolean; trip: Trip | null; stops: Stop[]; scheduleJson: string | null
}
...
export interface TripDetailFacade extends Subscribable<TripDetailState> { selectDay(day: number): void; selectStop(stopId: string | null): void }
