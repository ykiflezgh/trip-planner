...
async function boot() {
  ...
  // DEV-gated (the suite runs on the dev server): a production bundle can never be pointed at emulators or expose the test hooks.
  if (import.meta.env.DEV && import.meta.env.VITE_USE_EMULATORS === '1') await (await import('./lib/emulators')).connectEmulators()
  ...
  // StrictMode always, the e2e dev server included: its mount/unmount/mount replay is what useFacade is written for.
