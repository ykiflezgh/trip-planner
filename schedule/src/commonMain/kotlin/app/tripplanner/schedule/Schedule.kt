...
/**
 * Schedule engine (design v1.2 §6.5). One pure function turns a day's events, its hours and
 * the travel legs into timed calendar events plus warnings. Computed times are never stored:
 * every device and the ICS feed derive them from the same data with this same code.
 *
 * The engine never sees a place: an event carries at most a [EventInput.stopId], which is all
 * the travel lookup needs (design §7, decision 7).
 *
...
/** One event as the engine sees it, already in `order`. */
data class EventInput(
    val id: String,
    val title: String,
    val durationMin: Int,
    /** Id of the stop this event contains, `null` for an event without a place (no travel, not on the map). */
    val stopId: String? = null,
    /** Pinned wall-clock start in the trip zone (design §7: stored as "HH:mm"). */
    val fixedStart: LocalTime? = null,
    /** Travel mode to the next event with a stop; `null` = trip default. */
    val modeToNext: TravelMode? = null,
) {
    val hasStop: Boolean get() = stopId != null
}
...
sealed interface Warning {
    val eventId: String
    /** The cursor reached a pinned event after its start. */
    data class LateArrival(override val eventId: String, val minutes: Int) : Warning
    /** A pinned event starts before the previous event ends. */
    data class Overlap(override val eventId: String, val minutes: Int) : Warning
    /** The last event ends after the day's end time. */
    data class DayOverrun(override val eventId: String, val minutes: Int) : Warning
}
...
data class TimedEvent(
    ...
    val start: LocalDateTime,
    val end: LocalDateTime,
    /** The leg that precedes this event, when it and the previous stop-bearing event both contain stops. */
...
data class DaySchedule(
    ...
    val end: LocalDateTime get() = events.lastOrNull()?.end ?: LocalDateTime(date, hours.start)
}
...
object Schedule {
     ...
     *
     * Complexity:
     * - **Time:** O(E) for the E events of the day (one travel lookup per stop adjacency).
     * - **Space:** O(E) for the timed events and warnings.
     */
    fun computeDay(
        date: LocalDate,
        hours: DayHours,
        ...
        val out = ArrayList<TimedEvent>(events.size)
        val warnings = ArrayList<Warning>()
        var cursor = LocalDateTime(date, hours.start)
        var previousWithStop: EventInput? = null
        var previousEnd: LocalDateTime? = null

        for (event in events) {
            // Rule 2: travel from the previous event that contains a stop; events without one don't break adjacency.
            var leg: TravelLeg? = null
            val from = previousWithStop
            if (event.stopId != null && from?.stopId != null) {
                val mode = from.modeToNext ?: defaultMode
                val seconds = travel.seconds(from.stopId, event.stopId, mode)
                val legStart = cursor
                cursor = cursor.plusMinutes(((seconds ?: 0) + 59) / 60)
                leg = TravelLeg(from.stopId, event.stopId, mode, seconds ?: 0, pending = seconds == null, start = legStart, end = cursor)
            }
            // Rules 3-4: pinned events anchor the cursor; late arrivals and free time are reported.
            var gap = 0
            val start = event.fixedStart?.let { pinned ->
                val pinnedAt = LocalDateTime(date, pinned)
                val delta = cursor.minutesUntil(pinnedAt)
                if (delta < 0) warnings += Warning.LateArrival(event.id, -delta) else gap = delta
                // Rule 5: overlap with what came before.
                previousEnd?.let { pe -> if (pinnedAt < pe) warnings += Warning.Overlap(event.id, pinnedAt.minutesUntil(pe)) }
                pinnedAt
            } ?: cursor
            val end = start.plusMinutes(event.durationMin.coerceIn(MIN_DURATION_MIN, MAX_DURATION_MIN))
            out += TimedEvent(event, start, end, leg, gap)
            cursor = end
            previousEnd = end
            if (event.stopId != null) previousWithStop = event
        }
        // Rule 5: day overrun.
...
object ScheduleJs {
    ...
    fun computeDay(
        ...
        travelJson: String,
        defaultMode: String,
    ): String = ScheduleJson.computeDay(dateIso, dayStart, dayEnd, eventsJson, travelJson, defaultMode)
}
