...
class ScheduleTest {
    ...
    private fun place(id: String, min: Int, pinned: String? = null, mode: TravelMode? = null) =
        EntryInput(id, id, EntryKind.PLACE, min, Schedule.parseTime(pinned), mode)
    private fun custom(id: String, min: Int, pinned: String? = null) = EntryInput(id, id, EntryKind.CUSTOM, min, Schedule.parseTime(pinned))
    private fun t(s: String) = LocalDateTime.parse("2026-09-22T$s")

    @Test
    fun chainsDurationsAndTravelFromTheDayStart() {
        val legs = mapOf(Triple("a", "b", TravelMode.DRIVING) to 754, Triple("b", "c", TravelMode.WALKING) to 61)
        ...
        assertEquals(listOf(t("09:00"), t("10:43"), t("11:45")), day.entries.map { it.start })
        assertEquals(listOf(t("10:30"), t("11:43"), t("12:15")), day.entries.map { it.end })
        assertEquals(TravelMode.WALKING, day.entries[2].travelBefore!!.mode)
        assertTrue(day.warnings.isEmpty())
    }

    @Test
    fun customEntriesDoNotBreakTravelAdjacency() {
        val legs = mapOf(Triple("a", "b", TravelMode.DRIVING) to 600)
        ...
        assertNull(day.entries[1].travelBefore)
        val leg = day.entries[2].travelBefore!!
        assertEquals("a" to "b", leg.fromStopId to leg.toStopId)
        assertEquals(t("10:45"), leg.start)
        assertEquals(t("10:55"), day.entries[2].start)
    }
    ...
    fun missingTravelIsZeroAndPending() {
        ...
        assertEquals(t("10:00"), day.entries[1].start)
    }
    ...
    fun pinnedEntryAnchorsWithGapAndLateArrival() {
        ...
        assertEquals(t("14:00"), early.entries[1].start)
        assertEquals(210, early.entries[1].gapBeforeMin) // 10:30 -> 14:00
        ...
        assertEquals(t("14:00"), late.entries[1].start)
    ...
    fun durationsAreClamped() {
        ...
        assertEquals(5, day.entries[0].start.minutesUntil(day.entries[0].end))
        assertEquals(1440, day.entries[1].start.minutesUntil(day.entries[1].end))
    }
    ...
    fun dstEdgesResolveAtTheBoundaryOnly() {
        ...
        assertEquals(LocalDateTime(spring, LocalTime(2, 30)), day.entries[0].end)
        // 00:30 UTC and 02:30 (which is 01:30 UTC after the gap): the instant span is 60 min, not 120.
        assertEquals(3600, Schedule.epochSeconds(day.entries[0].end, "Europe/Lisbon") - Schedule.epochSeconds(day.entries[0].start, "Europe/Lisbon"))
    }
    ...
    fun jsonFacadeMatchesTheEngine() {
        val out = ScheduleJson.computeDay(
            ...
            """[{"id":"a","name":"A","kind":"place","durationMin":90},{"id":"b","name":"B","kind":"place","durationMin":60,"fixedStart":"11:00"}]""",
